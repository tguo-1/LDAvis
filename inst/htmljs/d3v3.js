d3v3 = function() {
  ...
  }, d3v3_selectMatcher = d3v3_documentElement[d3v3_vendorSymbol(d3v3_documentElement, "matchesSelector")], d3v3_selectMatches = function(n, s) {
  ...
  function d3v3_selection_creator(name) {
    return typeof name === "function" ? name : (name = d3v3.ns.qualify(name)).local ? function() {
      return d3v3_document.createElementNS(name.space, name.local);
    } : function() {
      return d3v3_document.createElementNS(this.namespaceURI, name);
    };
  }
  ...
  d3v3_selectionPrototype.transition = function() {
    var id = d3v3_transitionInheritId || ++d3v3_transitionId, subgroups = [], subgroup, node, transition = d3v3_transitionInherit || {
      time: Date.now(),
  ...
  var d3v3_event_dragSelect = d3v3_vendorSymbol(d3v3_documentElement.style, "userSelect"), d3v3_event_dragId = 0;
  function d3v3_event_dragSuppress() {
    var name = ".dragsuppress-" + ++d3v3_event_dragId, touchmove = "touchmove" + name, selectstart = "selectstart" + name, dragstart = "dragstart" + name, click = "click" + name, w = d3v3.select(d3v3_window).on(touchmove, d3v3_eventPreventDefault).on(selectstart, d3v3_eventPreventDefault).on(dragstart, d3v3_eventPreventDefault), style = d3v3_documentElement.style, select = style[d3v3_event_dragSelect];
    style[d3v3_event_dragSelect] = "none";
    return function(suppressClick) {
      w.on(name, null);
      style[d3v3_event_dragSelect] = select;
      if (suppressClick) {
        function off() {
          w.on(click, null);
        }
  ...
  d3v3.behavior.drag = function() {
    var event = d3v3_eventDispatch(drag, "drag", "dragstart", "dragend"), origin = null, mousedown = dragstart(d3v3_noop, d3v3.mouse, "mousemove", "mouseup"), touchstart = dragstart(touchid, touchposition, "touchmove", "touchend");
    function drag() {
    ...
    function touchposition(parent, id) {
      return d3v3.touches(parent).filter(function(p) {
        return p.identifier === id;
      })[0];
    }
    function dragstart(id, position, move, end) {
      return function() {
        var target = this, parent = target.parentNode, event_ = event.of(target, arguments), eventTarget = d3v3.event.target, eventId = id(), drag = eventId == null ? "drag" : "drag-" + eventId, origin_ = position(parent, eventId), dragged = 0, offset, w = d3v3.select(d3v3_window).on(move + "." + drag, moved).on(end + "." + drag, ended), dragRestore = d3v3_event_dragSuppress();
        if (origin) {
        ...
        function ended() {
          ...
          dragRestore(dragged && d3v3.event.target === eventTarget);
          event_({
            type: "dragend"
          });
        }
  ...
  d3v3.behavior.zoom = function() {
    var translate = [ 0, 0 ], translate0, scale = 1, scaleExtent = d3v3_behavior_zoomInfinity, mousedown = "mousedown.zoom", mousemove = "mousemove.zoom", mouseup = "mouseup.zoom", event = d3v3_eventDispatch(zoom, "zoom"), x0, x1, y0, y1, touchtime;
    function zoom() {
      this.on(mousedown, mousedowned).on(d3v3_behavior_zoomWheel + ".zoom", mousewheeled).on(mousemove, mousewheelreset).on("dblclick.zoom", dblclicked).on("touchstart.zoom", touchstarted);
    }
    ...
    function mousedowned() {
      var target = this, event_ = event.of(target, arguments), eventTarget = d3v3.event.target, dragged = 0, w = d3v3.select(d3v3_window).on(mousemove, moved).on(mouseup, ended), l = location(d3v3.mouse(target)), dragRestore = d3v3_event_dragSuppress();
      function moved() {
        dragged = 1;
      ...
      function ended() {
        w.on(mousemove, d3v3_window === target ? mousewheelreset : null).on(mouseup, null);
        dragRestore(dragged && d3v3.event.target === eventTarget);
      }
    }
    function touchstarted() {
      var target = this, event_ = event.of(target, arguments), touches = d3v3.touches(target), locations = {}, distance0 = 0, scale0 = scale, now = Date.now(), name = "zoom-" + d3v3.event.changedTouches[0].identifier, touchmove = "touchmove." + name, touchend = "touchend." + name, w = d3v3.select(d3v3_window).on(touchmove, moved).on(touchend, ended), t = d3v3.select(target).on(mousedown, null), dragRestore = d3v3_event_dragSuppress();
      ...
      function moved() {
        var touches = d3v3.touches(target), p0 = touches[0], l0 = locations[p0.identifier];
        if (p1 = touches[1]) {
          var p1, l1 = locations[p1.identifier], scale1 = d3v3.event.scale;
          if (scale1 == null) {
  ...
  var π = Math.PI, ε = 1e-6, ε2 = ε * ε, d3v3_radians = π / 180, d3v3_degrees = 180 / π;
  function d3v3_sgn(x) {
  ...
  function d3v3_rgbNumber(value) {
    return d3v3_rgb(value >> 16, value >> 8 & 255, value & 255);
  }
  ...
  d3v3_rgbPrototype.brighter = function(k) {
    ...
    if (g && g < i) g = i;
    if (b && b < i) b = i;
    return d3v3_rgb(Math.min(255, ~~(r / k)), Math.min(255, ~~(g / k)), Math.min(255, ~~(b / k)));
  };
  d3v3_rgbPrototype.darker = function(k) {
    k = Math.pow(.7, arguments.length ? k : 1);
    return d3v3_rgb(~~(k * this.r), ~~(k * this.g), ~~(k * this.b));
  };
  ...
  d3v3.timer = function(callback, delay, then) {
    ...
    if (d3v3_timer_queueTail) d3v3_timer_queueTail.next = timer; else d3v3_timer_queueHead = timer;
  ...
  function d3v3_timer_replace(callback, delay, then) {
    var n = arguments.length;
    if (n < 2) delay = 0;
    if (n < 3) then = Date.now();
    d3v3_timer_active.callback = callback;
    d3v3_timer_active.time = then + delay;
  }
  function d3v3_timer_mark() {
    var now = Date.now();
    d3v3_timer_active = d3v3_timer_queueHead;
    while (d3v3_timer_active) {
      if (now >= d3v3_timer_active.time) d3v3_timer_active.flush = d3v3_timer_active.callback(now - d3v3_timer_active.time);
      d3v3_timer_active = d3v3_timer_active.next;
    }
    return now;
  }
  function d3v3_timer_sweep() {
    ...
    while (t1) {
      if (t1.flush) {
        t1 = t0 ? t0.next = t1.next : d3v3_timer_queueHead = t1.next;
      } else {
  ...
  var d3v3_format_decimalPoint = ".", d3v3_format_thousandsSeparator = ",", d3v3_format_grouping = [ 3, 3 ], d3v3_format_currencySymbol = "$";
  ...
  d3v3.format = function(specifier) {
    var match = d3v3_format_re.exec(specifier), fill = match[1] || " ", align = match[2] || ">", sign = match[3] || "", symbol = match[4] || "", zfill = match[5], width = +match[6], comma = match[7], precision = match[8], type = match[9], scale = 1, suffix = "", integer = false;
    ...
    if (symbol === "#") symbol = ""; else if (symbol === "$") symbol = d3v3_format_currencySymbol;
    ...
    return function(value) {
      ...
      var i = value.lastIndexOf("."), before = i < 0 ? value : value.substring(0, i), after = i < 0 ? "" : d3v3_format_decimalPoint + value.substring(i + 1);
      if (!zfill && comma) before = d3v3_format_group(before);
      var length = symbol.length + before.length + after.length + (zcomma ? 0 : negative.length), padding = length < width ? new Array(length = width - length + 1).join(fill) : "";
      if (zcomma) before = d3v3_format_group(padding + before);
      negative += symbol;
      value = before + after;
  ...
  if (d3v3_format_grouping) {
    ...
    d3v3_format_group = function(value) {
      var i = value.length, t = [], j = 0, g = d3v3_format_grouping[0];
      while (i > 0 && g > 0) {
        t.push(value.substring(i -= g, i + g));
        g = d3v3_format_grouping[j = (j + 1) % d3v3_format_groupingLength];
      }
      return t.reverse().join(d3v3_format_thousandsSeparator);
    };
  }
  d3v3.geo = {};
  function d3v3_adder() {}
  ...
  function d3v3_geo_clip(pointVisible, clipLine, interpolate, polygonContains) {
    return function(listener) {
      ...
      var segments;
      var buffer = d3v3_geo_clipBufferListener(), ringListener = clipLine(buffer), polygon, ring;
  ...
  function d3v3_geo_pointInPolygon(point, polygon) {
    ...
    for (var i = 0, n = polygon.length; i < n; ++i) {
      ...
      while (true) {
        ...
        d3v3_geo_areaRingSum.add(Math.atan2(k * Math.sin(dλ), cosφ0 * cosφ + k * Math.cos(dλ)));
    ...
    return (!southPole && !polar && d3v3_geo_areaRingSum < 0 || polarAngle < -ε) ^ winding & 1;
  }
  var d3v3_geo_clipAntimeridian = d3v3_geo_clip(d3v3_true, d3v3_geo_clipAntimeridianLine, d3v3_geo_clipAntimeridianInterpolate, d3v3_geo_clipAntimeridianPolygonContains);
  ...
  var d3v3_geo_clipAntimeridianPoint = [ -π, 0 ];
  function d3v3_geo_clipAntimeridianPolygonContains(polygon) {
    return d3v3_geo_pointInPolygon(d3v3_geo_clipAntimeridianPoint, polygon);
  }
  function d3v3_geo_clipCircle(radius) {
    var cr = Math.cos(radius), smallRadius = cr > 0, point = [ radius, 0 ], notHemisphere = Math.abs(cr) > ε, interpolate = d3v3_geo_circleInterpolate(radius, 6 * d3v3_radians);
    return d3v3_geo_clip(visible, clipLine, interpolate, polygonContains);
    ...
    function polygonContains(polygon) {
      return d3v3_geo_pointInPolygon(point, polygon);
    }
  }
  ...
  function d3v3_geo_projectionMutator(projectAt) {
    ...
    projection.stream = function(output) {
      ...
      stream = d3v3_geo_projectionRadiansRotate(rotate, preclip(projectResample(postclip(output))));
      stream.valid = true;
      return stream;
    };
  ...
  d3v3.geom.voronoi = function(points) {
    var x = d3v3_svg_lineX, y = d3v3_svg_lineY, clipPolygon = null;
    if (arguments.length) return voronoi(points);
    function voronoi(data) {
      var points, polygons = data.map(function() {
        return [];
      }), fx = d3v3_functor(x), fy = d3v3_functor(y), d, i, n = data.length, Z = 1e6;
      if (fx === d3v3_svg_lineX && fy === d3v3_svg_lineY) points = data; else for (points = new Array(n), 
      i = 0; i < n; ++i) {
    ...
    voronoi.triangles = function(data) {
      ...
      var points = new Array(n), fx = d3v3_functor(x), fy = d3v3_functor(y), d, i = -1, n = data.length;
      while (++i < n) {
  ...
  d3v3.layout.tree = function() {
    ...
    function tree(d, i) {
      ...
      d3v3_layout_treeVisitAfter(root, nodeSize ? function(node) {
        node.x *= size[0];
  ...
  function d3v3_scale_linearNice(domain, m) {
    return d3v3_scale_nice(domain, d3v3_scale_niceStep(m ? d3v3_scale_linearTickRange(domain, m)[2] : d3v3_scale_linearNiceStep(domain)));
  }
  function d3v3_scale_linearNiceStep(domain) {
    var extent = d3v3_scaleExtent(domain), span = extent[1] - extent[0];
  ...
  function d3v3_scale_log(linear, base, positive, domain) {
    ...
    scale.ticks = function() {
      var extent = d3v3_scaleExtent(domain), ticks = [];
      ...
      for (var group = this[j], i = -1, n = group.length; ++i < n; ) {
        if (node = group[i]) {
          ...
          for (var k = -1, o = subnodes.length; ++k < o; ) {
            if (subnode = subnodes[k]) d3v3_transitionNode(subnode, k, id, transition);
            subgroup.push(subnode);
          }
        }
      }
  ...
  d3v3_transitionPrototype.filter = function(filter) {
    ...
    return d3v3_transition(subgroups, this.id);
  };
  ...
  d3v3_transitionPrototype.each = function(type, listener) {
    ...
    if (arguments.length < 2) {
      ...
      d3v3_selection_each(this, function(node) {
        ...
        (transition.event || (transition.event = d3v3.dispatch("start", "end"))).on(type, listener);
      });
    }
    return this;
  };
  ...
  function d3v3_transitionNode(node, i, id, inherit) {
    ...
    if (!transition) {
      ...
      d3v3.timer(function(elapsed) {
        ...
        d3v3_timer_replace(start, delay, time);
        function start(elapsed) {
          ...
          d3v3_timer_replace(tick, 0, time);
        }
  ...
  d3v3.svg.brush = function() {
    var event = d3v3_eventDispatch(brush, "brushstart", "brush", "brushend"), x = null, y = null, resizes = d3v3_svg_brushResizes[0], extent = [ [ 0, 0 ], [ 0, 0 ] ], clamp = [ true, true ], extentDomain;
    function brush(g) {
      g.each(function() {
    ...
    function brushstart() {
      var target = this, eventTarget = d3v3.select(d3v3.event.target), event_ = event.of(target, arguments), g = d3v3.select(target), resizing = eventTarget.datum(), resizingX = !/^(n|s)$/.test(resizing) && x, resizingY = !/^(e|w)$/.test(resizing) && y, dragging = eventTarget.classed("extent"), dragRestore = d3v3_event_dragSuppress(), center, origin = mouse(), offset;
  ...
  var d3v3_time_dayRe = d3v3_time_formatRe(d3v3_time_days), d3v3_time_dayLookup = d3v3_time_formatLookup(d3v3_time_days), d3v3_time_dayAbbrevRe = d3v3_time_formatRe(d3v3_time_dayAbbreviations), d3v3_time_dayAbbrevLookup = d3v3_time_formatLookup(d3v3_time_dayAbbreviations), d3v3_time_monthRe = d3v3_time_formatRe(d3v3_time_months), d3v3_time_monthLookup = d3v3_time_formatLookup(d3v3_time_months), d3v3_time_monthAbbrevRe = d3v3_time_formatRe(d3v3_time_monthAbbreviations), d3v3_time_monthAbbrevLookup = d3v3_time_formatLookup(d3v3_time_monthAbbreviations), d3v3_time_percentRe = /^%/;
  ...
  function d3v3_time_parseWeekdayNumber(date, string, i) {
    ...
    var n = d3v3_time_numberRe.exec(string.substring(i, i + 1));
    return n ? (date.w = +n[0], i + n[0].length) : -1;
  }
  function d3v3_time_parseWeekNumberSunday(date, string, i) {
    d3v3_time_numberRe.lastIndex = 0;
    var n = d3v3_time_numberRe.exec(string.substring(i));
  ...
  function d3v3_time_parseLiteralPercent(date, string, i) {
    ...
    var n = d3v3_time_percentRe.exec(string.substring(i, i + 1));
  ...
  function d3v3_time_scale(linear, methods, format) {
    ...
    scale.nice = function(m) {
      return scale.domain(d3v3_scale_nice(scale.domain(), m));
    };
    scale.ticks = function(m, k) {
      ...
      if (typeof m !== "function") {
        ...
        if (target / d3v3_time_scaleSteps[i - 1] < d3v3_time_scaleSteps[i] / target) --i;
        m = methods[i];
        k = m[1];
        m = m[0].range;
      }
